...
class AiTriageRepositoryImpl(
    ...
    override suspend fun analyzePainPoints(
        ...
        userNotes: String
    ): Result<ClinicalTriageReport> = withContext(Dispatchers.IO) {
    ...
    override suspend fun saveTriageReport(report: ClinicalTriageReport): Result<Unit> = withContext(Dispatchers.IO) {
        _latestReport.value = report
        Result.success(Unit)
    }
}
